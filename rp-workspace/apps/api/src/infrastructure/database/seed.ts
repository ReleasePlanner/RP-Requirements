...
async function seed() {
    ...
    try {
        ...
        const riskLevels = await dataSource.getRepository(RiskLevel).save([
        ...
        const complexities = await dataSource.getRepository(Complexity).save([
            { complexityId: 1, name: 'High' }, { complexityId: 2, name: 'Medium' }, { complexityId: 3, name: 'Low' }
        ]);
        const effortTypes = await dataSource.getRepository(EffortEstimateType).save([
            { effortEstimateTypeId: 1, name: 'Story Points' }, { effortEstimateTypeId: 2, name: 'Hours' }
        ]);
        const sources = await dataSource.getRepository(Source).save([
            { sourceId: 1, name: 'Customer' }, { sourceId: 2, name: 'Market Analysis' }
        ]);
        const metrics = await dataSource.getRepository(Metric).save([
            { metricId: 1, name: 'NPS Impact' }, { metricId: 2, name: 'Revenue Growth' }
        ]);

        console.log('�👥 Seeding Sponsors...');
        const password = await bcrypt.hash('Password123!', 10);
        const sponsors = await dataSource.getRepository(Sponsor).save([
            { sponsorId: uuidv4(), name: 'System Admin', email: 'admin@example.com', password, role: 'Admin' },
            { sponsorId: uuidv4(), name: 'Sarah Connor', email: 'sponsor@example.com', password, role: 'Sponsor' },
            { sponsorId: uuidv4(), name: 'John Doe', email: 'po@example.com', password, role: 'ProductOwner' },
            { sponsorId: uuidv4(), name: 'Jane Smith', email: 'approver@example.com', password, role: 'Approver' },
        ]);
        ...
        const productOwners = await productOwnerRepo.save([
            { productOwnerId: uuidv4(), firstName: 'Alice', lastName: 'Johnson', role: 'Lead PO', status: 'Active' },
            { productOwnerId: uuidv4(), firstName: 'Bob', lastName: 'Williams', role: 'Technical PO', status: 'Active' },
            { productOwnerId: uuidv4(), firstName: 'Charlie', lastName: 'Brown', role: 'Junior PO', status: 'Inactive' },
        ]);

        console.log('📝 Seeding Approvers...');
        const approverRepo = dataSource.getRepository(Approver);
        const approvers = await approverRepo.save([
            { approverId: uuidv4(), firstName: 'David', lastName: 'Miller', role: 'Director', status: 'Active' },
            { approverId: uuidv4(), firstName: 'Eva', lastName: 'Davis', role: 'VP', status: 'Active' },
            { approverId: uuidv4(), firstName: 'Frank', lastName: 'Moore', role: 'Manager', status: 'Active' },
            { approverId: uuidv4(), firstName: 'Grace', lastName: 'Taylor', role: 'Senior Manager', status: 'Active' },
            { approverId: uuidv4(), firstName: 'Henry', lastName: 'Anderson', role: 'Head of Product', status: 'Active' },
            { approverId: uuidv4(), firstName: 'Ivy', lastName: 'Thomas', role: 'CTO', status: 'Active' },
            { approverId: uuidv4(), firstName: 'Jack', lastName: 'Jackson', role: 'CIO', status: 'Inactive' },
            { approverId: uuidv4(), firstName: 'Kathy', lastName: 'White', role: 'Manager', status: 'Active' },
            { approverId: uuidv4(), firstName: 'Leo', lastName: 'Harris', role: 'Director', status: 'Inactive' },
        ...
        const createdReq = await reqRepo.save({
            ...
            effortEstimate: 13,
            actualEffort: 5,
            businessValue: 80.00, // High Value
        ...
        await reqRepo.save({
            ...
            businessValue: 90.00, // High Value -> Strategic Bet
        });
        ...
        await reqRepo.save({
            ...
            businessValue: 95.00, // High Value -> Quick Win
        });
        ...
        await reqRepo.save({
            ...
            businessValue: 10.00, // Low Value -> Time Sink
        });
        ...
        await reqRepo.save({
            ...
            businessValue: 5.00, // Low Value -> Filler
        });
